...
class model(nn.Module):
    ...
    def forward(self, x):
        # Permute input from (N, H, W, C) to (N, C, H, W) for PyTorch Conv2d layers
        x = x.permute(0, 3, 1, 2)
        ...
        x = self.fc2(x)
        return x
# Define a small cnn model such that it has enough apperture to achieve universal approximation theorem
